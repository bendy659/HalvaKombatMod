...
import ru.hollowhorizon.hc.client.imgui.ImGuiMethods.progressBar
...
class HalvaKombatMenu(pPlayer: Player) : HollowScreen() {

  /* Variables */

  var progressLoading = ImFloat(0f)

  var score = HalvaKombatData.score
  var nextScore = HalvaKombatData.nextScore
  var pointUpgrades = HalvaKombatData.pointsUpgrade
  var perClick = HalvaKombatData.perClick
  var clickUpgradeCost = HalvaKombatData.clickUpgradeCost
  ...
  private fun mainMenu() {
    ...
    ImGui.pushStyleColor(ImGuiCol.FrameBg, 25, 49, 156, 255)
    progressBar((score.get() / nextScore.get()).toFloat(), ImGui.getWindowWidth(), 32f)
    ImGui.popStyleColor()
    ...
    ImGui.setCursorPos(32f, 650f)

    ImGui.pushStyleColor(ImGuiCol.Button, 25, 49, 156, 255)

    if (ImGui.button("Клик+ | $clickUpgradeCost", 300f, 120f) && score.get() >= clickUpgradeCost.get()) {
      score.set(score.get() - clickUpgradeCost.get())
      pointUpgrades.set(pointUpgrades.get() - 1)
      clickUpgradeCost.set((clickUpgradeCost.get() * 1.65).roundToInt())

      perClick.set(perClick.get() + 1)
    }
  ...
  private fun nextScoreUpd() {
    if(score.get() >= nextScore.get()) {
      val newNextScore = ImInt(round(nextScore.get() * 2 * 1.35).toInt())
  ...
  override fun keyPressed(pKeyCode: Int, pScanCode: Int, pModifiers: Int): Boolean {
    if (pKeyCode == GLFW.GLFW_KEY_ESCAPE) {
      HalvaKombatData.score = this.score
      HalvaKombatData.nextScore = this.nextScore
      HalvaKombatData.pointsUpgrade = this.pointUpgrades
      HalvaKombatData.perClick = this.perClick
      HalvaKombatData.clickUpgradeCost = this.clickUpgradeCost
    }
