...
class NewNPCCreatorGUI(val npc: NPCEntity, private val npcID: Int, var isEditor: Boolean = false): HollowScreen() {
  ...
  override fun render(pPoseStack: PoseStack, pMouseX: Int, pMouseY: Int, pPartialTick: Float) {
    ...
    ImguiHandler.drawFrame() {
      ...
      if (
        ...
        val tPosXYZ = arrayOf(transformPos[0].toFloat(), transformPos[1].toFloat(), transformPos[2].toFloat())
        val tRotXYZ = arrayOf(transformRot[0].toFloat(), transformRot[1].toFloat(), transformRot[2].toFloat())
        val tSclXYZ = arrayOf(transformScl[0].toFloat(), transformScl[1].toFloat(), transformScl[2].toFloat())
  ...
  private fun transformation() {
    ...
    if (isChange) npc[AnimatedEntityCapability::class].transform = Transform(
      transformPos[0].toFloat(), transformPos[1].toFloat(), transformPos[2].toFloat(), // Position: X Y Z
      transformRot[0].toFloat(), transformRot[1].toFloat(), transformRot[2].toFloat(), // Rotation: X Y Z
      transformScl[0].toFloat(), transformScl[1].toFloat(), transformScl[2].toFloat() // Scale: X Y Z
    )
  }
...
class NPCData(
  ...
  override fun handle(player: Player, data: NPCData) {
    ...
    if (entity == null) {
      player.sendSystemMessage(HelperPack.lang("error").mcTranslate())
      return
    }
